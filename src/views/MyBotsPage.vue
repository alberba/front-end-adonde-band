...
import type { Bot, BotLeagueSummary, League, Participation } from '@/types'
...
async function loadBotDetails() {
  console.log(botSummaries.value)
  ...
  for (const botsummary of botSummaries.value) {
    ...
    const response = await fetch(`http://localhost:8080/api/v0/bot/${botsummary.id}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${localStorage.getItem('token')}`,
...
function extractPositions(leaderboard: Participation[], botId: number | undefined) {
...
<template>
  ...
  <main v-if="isLoading" class="flex h-screen w-full items-center justify-center">Cargando...</main>
  <main
    v-else
  ...
  <JoinLeagueModal v-if="visibleJoinLeagueModal" :selectedBotId="selectedBotId" @close-join-league-modal="visibleJoinLeagueModal = false" />
  <FooterApp>
    <img
